...
namespace DaggerfallConnect.Arena2
{
    ...
    public abstract class BaseImageFile
    {
        ...
        /// Get the raw bytes
        public byte[] GetBytesBGRA(DFBitmap srcBitmap, int alphaIndex, int border, out DFSize sizeOut, int emissionIndex = -1, int customAlphaValue = 255)
        {
            ...
            for (int y = 0; y < srcBitmap.Height; y++)
            {
                ...
                for (int x = 0; x < srcBitmap.Width; x++)
                {
                    ...
                    else
                    {
                        ...
                        float S;
                        float V;
                        Color32.RGBToHSV(albedoColors[dstRow + borderSize + x], out H, out S, out V);
                        float emission = Mathf.Pow(V, 1.9f);
                        emissionColors[dstRow + borderSize + x] = Color32.Lerp(otherEmission, albedoColors[dstRow + borderSize + x], Mathf.Clamp01(emission));
                    }
                }
            }
